...
## 1. New Postgres schema (Cloudflare Hyperdrive)
Recommendation, not a toss-up: use a real Postgres instance behind Cloudflare Hyperdrive (a pooler/accelerator — you still need to provision an actual Postgres server, e.g. Neon/Supabase/self-managed, behind it), not the existing `DuckDBIncidentsManager` pattern (`services/agent/src/vss_agents/tools/incidents.py`) — that class is a read-only cache rebuilt from bulk-rescanned S3 JSON files, has no single-record write API, isn't wired into base profile's flow today, and its DuckDB connection is a per-process singleton unsuited to a separate dashboard app reading the same data concurrently. A real Postgres table gives the team the schema/field-name control it needs and is trivially shared between the agent (writer) and the new frontend/API (reader).

| Table | Purpose | Key fields |
|---|---|---|
| `videos` | Catalog of every ingested video, including upload and metadata-correction capabilities | `id`, `filename`, `r2_key`, `uploaded_at`, `status` (`unanalyzed`/`analyzing`/`analyzed`/`failed`), `location`, `camera_source`, `metadata_edited_by`, `metadata_edited_at`, `sensor_id` (VIOS reference) |
| `incident_reports` | One row per generated report, covering report generation + status transitions | `id`, `video_id` (FK), `incident_type`, `severity` (1–5), `confidence`, `incident_start`, `incident_end`, `incident_start_confirmed`, `description`, `persons` (jsonb), `location`, `status` (`unreviewed`/`verified`), `verified_by`, `verified_at`, `edited_by`, `edited_at`, `created_at` |
| `notifications` | High-severity alerts | `id`, `report_id`, `severity`, `created_at`, `acknowledged` |
| `severity_eval_log` | Human-vs-AI severity comparison (product-facing eval) | `id`, `report_id`, `ai_severity`, `human_severity`, `rater`, `rated_at` |

`verified_by`/`edited_by`/`rater` are freeform text (who typed their name in), not foreign keys to a user table — per the Overview doc's Auth decision (§5, Judgment Calls). Skip a migration framework (Alembic etc.) — capstone-scale schema, `CREATE TABLE IF NOT EXISTS` run once on startup is enough.

New file: `services/agent/src/vss_agents/utils/incident_db.py` — small async helper (`asyncpg` or SQLAlchemy-async) for CRUD against these tables.
...
**Verify live:** `asyncpg`/SQLAlchemy connects through Hyperdrive with the right `sslmode`; keep the app-side connection pool small since Hyperdrive already pools.
...
Per the Overview doc's Retention decision (§5, Judgment Calls), R2 is the permanent video store on both ends of the pipeline — not an archive tier:
...
New file `services/agent/src/vss_agents/data_models/incident_report.py` — defines the `IncidentReport` Pydantic schema. Add `persons: list[{description, actions}]`, extracted via `llm.with_structured_output` (same pattern already used in `services/agent/src/vss_agents/agents/postprocessing/validators/llm_based_rule_validator.py:131` and `services/agent/src/vss_agents/evaluators/report_evaluator/field_evaluators/llm_judge.py:246`). "States this rather than fabricating" is prompt engineering + an empty-list-is-valid schema, not new infra.

**Incident classification**
Add `incident_type`, `severity` (1–5), `confidence` (0–1) to `IncidentReport`. Proposed default taxonomy (edit freely, and not confirmed against any spec — check it): robbery, burglary, vandalism, assault/fighting, trespassing, suspicious behavior, weapon presence, other — drawn from the problem statement's own examples. Low-confidence flagging is a UI threshold on `confidence`, no backend change.
...
Add an LLM pass inside `incident_report_gen` that scans the same chunk-level timestamped captions `video_report_gen` already consumes, and returns the timestamp of the first chunk where the described incident actually begins — populating `incident_start`/`incident_end`. Falls back to `0:00` + `incident_start_confirmed=false` when not confidently identifiable. Built on existing timestamped-chunk infrastructure, not new video processing.

### Workflow & Automation

**One-click full pipeline**
`POST /api/v1/incidents/{id}/analyze` (§4) — internally chains ingestion-status check → (embedding-indexing wait, only meaningful once MVP2's embedding pipeline is running — no-op during MVP1) → `incident_report_gen`, per the Overview doc's pipeline-orchestration decision (§5, Judgment Calls). Progress visibility: since Streamlit talks to Postgres directly, it doesn't need a streaming response from this endpoint at all — `st.status()` wraps a rerun/polling loop reading `videos.status` directly from Postgres as the pipeline advances (`unanalyzed`→`analyzing`→`analyzed`/`failed`) — halting gracefully and surfacing the failure. On failure at any stage, the endpoint sets `videos.status='failed'` plus an error message column, which Streamlit then displays. **Under remote mode**, given the concurrent-request-ceiling risk flagged in your Implementation doc's §1, set an explicit timeout on the upstream LLM/VLM calls inside this chain — without one, a saturated remote endpoint can hang the pipeline indefinitely rather than surfacing as `failed`.

**Auto-populate structured report**
Exactly what `incident_report_gen`'s `IncidentReport` schema (Incident classification / start-time detection above) already produces — confirm its fields cover whatever the current report-card spec needs (`type`, `location`, `time range`, `description`, `severity`, `confidence` — all present as designed).

**Status transitions**
`incident_reports.status` defaults `'unreviewed'` on insert. Streamlit's "Verify" button issues a direct `UPDATE incident_reports SET status='verified', verified_by=..., verified_at=now()` — no AI involved, so no need to route through `vss-agent`. Dashboard queries (below) filter `status='verified'` only, so unreviewed reports don't appear in manager-facing analytics — check this against whatever the current spec says about that boundary.
...
- **Search/filter** → direct `SELECT ... FROM incident_reports WHERE ...` (type/status/keyword)
- **Jump-to-timestamp** → `st.video(url, start_time=incident_reports.incident_start)`; if `incident_start_confirmed=false`, show an "unconfirmed" badge, default to 0:00 (the start-time-detection fallback above)
- **Edit/verify UI** → direct `UPDATE incident_reports` (edit fields) + the verify update above, sets `edited_by`/`edited_at`
- **Dashboard (filters + insights)** — all read from a direct aggregate query over `incident_reports` (counts by type/location/time, top recurring type per location+time-bucket for the "insights" text), rendered with `st.plotly_chart`/`st.bar_chart`. Insights sentences can start as a templated string over the aggregate results — no new ML needed unless the team wants it more sophisticated later.
...
└── services/agent/
    ...
    └── src/vss_agents/
        ...
        │                                                          (persons, incident_type, severity,
        │                                                           confidence, incident_start/end, §5, Key
