...
async def test_get_video_returns_dict_when_found():
    mock_client = _make_mock_client()
    mock_client.table().select().maybe_single().execute = AsyncMock(return_value=MagicMock(data={"id": "vid-1", "filepath": "a.mp4"}))
    db = IncidentDB(mock_client)
...
async def test_list_videos_maps_all_rows():
    mock_client = _make_mock_client()
    mock_client.table().select().order().execute = AsyncMock(return_value=MagicMock(data=[{"id": "vid-1"}, {"id": "vid-2"}]))
    db = IncidentDB(mock_client)
...
@pytest.mark.asyncio
async def test_dead_methods_raise_not_implemented():
    """All dead methods (no production callers) raise NotImplementedError."""
    mock_client = _make_mock_client()
    db = IncidentDB(mock_client)

    dead_methods = [
        ("set_review_status", ("vid-1", "run-1"), {"status": "verified", "reviewed_by": "alice", "notify_threshold": 4}),
        ("list_incident_entities", ("vid-1",), {}),
        ("list_incident_instruments", ("vid-1",), {}),
        ("list_incident_assets", ("vid-1",), {}),
        ("add_incident_instrument", ("vid-1", "run-1"), {"instrument_id": "i1"}),
        ("add_incident_asset", ("vid-1", "run-1"), {"asset_id": "a1"}),
        ("insert_generated_report", ("report-1",), {"incident_id": "vid-1", "model_run_id": "run-1"}),
        ("list_notifications", (), {"only_unacknowledged": False}),
        ("acknowledge_notification", (42,), {}),
        ("get_review_status", ("vid-1", "run-1"), {}),
    ]

    for method_name, args, kwargs in dead_methods:
