...
`dashboard_seed.py` no longer hardcodes an incident list. `load_seed()`
...
| File | Rows |
|---|---|
| `data/incidents.csv` | 72 |
| `data/entities.csv` | 133 |
| `data/instruments.csv` | 37 |
| `data/assets.csv` | 40 |
...
seconds, `Severity`, and `Confidence_Score` omitted entirely when blank). `Video`
rows are still generated in code, one per incident (`ID=V<n>`), because the sheet
provides no Video data. Each generated row carries the matching `Incident_ID` and
...
assigned to rows with no matching object when the bucket has fewer clips than the
fixture. `Report` and `Query` stay empty lists.
...
## Composition (72 sample incidents)

**Transcribed rows (36 incidents)** — the capstone group's ground-truth sheets,
...
- All `Severity_Level` values for `burglary` and `road accident` are blank
  in the sheet and are carried through as `None`.
- Every transcribed `Confidence_Score` is blank → the key is omitted on those rows.
- Every transcribed `Image` is blank → `Image` is `None` on all entities/instruments/assets.

**Generated rows (36 incidents)** — filling gaps the sheets lack. IDs use the
`SYN-<Type><NNN>` scheme (`SYN-Fighting001`, `SYN-Explosion001`, …);
entity/instrument/asset IDs stay `E1` / `I1` / `A1` scoped to their incident.

- 12 `fighting` (the enum has the type; the sheets have zero fighting incidents).
- 9 `explosion`, each with linked entities, instruments **and** assets (the sheets
  have explosion incidents but no linked rows of any kind for them).
- 5 `burglary`, 5 `road accident`, 5 `animal` to round out the spread.
- A wide `Confidence_Score` spread (the sheets have none): several scores below
...
- Broader instrument/asset variety across all types, with `Threat_Level` per the
  same rubric the sheets use (firearm/IED-class 5, blade/machete 4,
  bat/pipe/vehicle 3, screwdriver/bottle/bolt-cutter 2, trolley/bag 1;
  road-accident vehicles left blank, matching the transcribed rows).
- Every `SYN-` incident has 1–4 linked entities (human / animal / unknown mix)
  and, where it fits the scenario, instruments (with `Entity_ID` and
  `Threat_Level`) and assets. Descriptions match the transcribed style: short,
  objective, lowercase.

## Known ground-truth gaps (carried through as-is, not invented)

Reproduced from `groundtruth/NOTES.md`; these affect only the transcribed
(non-`SYN-`) rows:
...
2. Blank `Severity_Level` on every transcribed `burglary` and `road accident`
   incident → carried as `None`. These incidents do not appear in severity-filtered
   dashboard views or the "Critical Alerts (Sev 4–5)" KPI, and show "Not
   supplied" in report detail. Do **not** invent values.
3. `Confidence_Score` blank for every transcribed row → key omitted (never `None`/`0`).
4. `Image` blank for every transcribed entity/instrument/asset → `None`.
...
10. Instrument names like `suv` / `truck` / `bus` / `car` / `van` are vehicles the
    group recorded as "instruments" (the means used); kept as-is. The `SYN-`
    rows follow the same convention.
11. No transcribed `explosion` entities/instruments/assets and no transcribed
    `fighting` incidents were provided — the `SYN-` rows supply both.
