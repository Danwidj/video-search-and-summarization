...
def test_same_seed_and_isolated_edits():
    ...
    assert len(handle.list_reports()) == len(seed) == 18
    assert handle.get_report(seed[0]["Incident_ID"])["description"] == seed[0]["Description"]
    handle.update_report("MOCK-001", fields={"description": ""}, edited_by="Reviewer")
...
def test_player_uses_explicit_url_and_seed_timestamp():
    ...
    assert not app.exception
    assert app.get("video")[0].proto.start_time == 12
