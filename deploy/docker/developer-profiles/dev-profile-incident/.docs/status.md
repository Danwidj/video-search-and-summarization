...
|---|---|---|
| **incident-console-v2 (Next.js)** | **Done (pagination live-verified 2026-09-26; report editor implementation 2026-09-27)** | `incident-console-v2/` (App Router UI, chunked VST upload, R2 direct fallback, report workspace, review flow, notification banner, eval form). `/reports` requests six lightweight summaries through `list_incident_report_summaries`, lazy-loads small upload-time R2 screenshots with a visible skeleton fallback, shows the current result range above centered pagination, and defers full report/video loading to the selected report route. New thumbnails sample the frame 25% into the video; this replaced opening-frame sampling after a valid animal thumbnail was observed to be all black. Existing R2 videos have no thumbnails unless separately backfilled. On an individual report, `Edit report` opens an inline workspace showing the original AI report beside the editable copy. Title, incident type, summary, severity, confidence, time range, duration, location, severity reason, people/entities, instruments, assets, timeline, and uncertainties are editable. The full edited copy is stored separately in model-run notes; the original AI report remains preserved. The former bottom-page structured editor and `Start review` action are removed. Migration `20260926120000_list_incident_report_summaries.sql` was applied live on 2026-09-26; read-back returned 6 of 35 reports with no `playbackUrl`, and execute permission is restricted to `service_role`. Analysis durability was also live-verified 2026-09-26: upload `incident-ingestion-e2e-20260926.mp4` created R2 key `anomaly/road_accidents/incident-ingestion-e2e-20260926.mp4`, video `vc468d0b5487a1c4d814`, model run `ma45e70563428ae88220`, and report `r12abb87e701877fdfbd`. |
