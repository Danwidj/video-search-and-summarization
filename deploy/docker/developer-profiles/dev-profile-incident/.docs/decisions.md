...
- **Decision:** In `services/agent/src/vss_agents/tools/video_report_gen.py`, `lvs_video_understanding.py`, and `lvs_config_media.py`, catch `NotImplementedError` raised by NAT's `user_input_manager.prompt_user_input` when no human prompt callback is registered (e.g. during non-interactive REST API requests) and fall back cleanly to empty string / default prompt instead of raising HTTP 500. In `video_report_gen.py`, update `_filter_short_duration_from_markdown` to preserve original timestamped events if short-duration filtering (`min_duration_seconds=2.0`) would eliminate all events (e.g., short video clips where individual action spans are under 2 seconds), preventing empty markdown reports and spurious empty-incident fallbacks.
