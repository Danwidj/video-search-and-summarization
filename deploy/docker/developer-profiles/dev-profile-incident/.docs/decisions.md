...
- **Decision:** Remove `sdr-controller` and the SDRC services (`init-dirs`, `render-config`, `wdm-env-from-config`, `wait-for-redis`, `wait-for-docker-workloads`) from the incident profile deployment. Add `deploy/docker/developer-profiles/dev-profile-incident/compose.override.yml` setting `scale: 0` for all SDRC services and configure `start.sh` and `rebuild-svc.sh` to include `-f developer-profiles/dev-profile-incident/compose.override.yml` and `--scale init-dirs=0 --scale render-config=0 --scale wdm-env-from-config=0 --scale sdr-controller=0`. Retain the `SDR_CONTROLLER_CONFIG_PATH` variable in `dev-profile-incident/.env` (upstream `sdrc/docker-compose.yaml` included via `compose.yml` evaluates volume interpolation at parse time), and remove known issue 4 from `.docs/status.md`.
