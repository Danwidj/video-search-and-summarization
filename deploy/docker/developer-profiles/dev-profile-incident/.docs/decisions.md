...
- **Decision:** In `incident-console-v2/lib/analysis/prompt.ts`, replace example placeholders `"duration_seconds": 0` and `"incident_start"/"incident_end": "0:00"` with `null` so small reasoning models (`nvidia/cosmos-3-nano-reasoner`) are not primed to output zero duration or zero timestamps. In `incident-console-v2/lib/analysis/schema.ts`, mirror the agent's fallback logic (`incident_report_gen.py`): when `duration_seconds` is `null` or `0` and the timeline contains events with a positive span, derive `duration_seconds` from the rounded timeline span `Math.max(0, Math.round(lastEndOrStart - firstStart))`. Model stays on `nvidia/cosmos-3-nano-reasoner`.
