...
export function ReportsLibrary() {
  ...
  const types = useMemo(() => Array.from(new Set(reports.map((item) => item.incidentType))).sort(), [reports]);
  const filtered = useMemo(() => {
    ...
    const result = reports.filter((item) => {
      ...
      const haystack = `${item.title} ${item.filename} ${item.incidentType} ${item.summary} ${item.searchableEvidence}`.toLowerCase();
      return (!needle || haystack.includes(needle))
        && (type === 'all' || item.incidentType === type)
        && (severity === 'all' || (severity === 'high' ? item.severityLevel >= 4 : item.severityLevel === Number(severity)))
    ...
    return result.sort((a, b) => {
      ...
      if (sort === 'severity-high') return b.severityLevel - a.severityLevel;
      if (sort === 'severity-low') return a.severityLevel - b.severityLevel;
      if (sort === 'confidence-high') return b.confidenceScore - a.confidenceScore;
      if (sort === 'confidence-low') return a.confidenceScore - b.confidenceScore;
...
function ReportCard({ item, busy, onStatus, onReanalyze, onDelete, onOpen }: { item: ReportLibraryItem; busy: boolean; onStatus: (item: ReportLibraryItem, status: ReviewStatus) => void; onReanalyze: (item: ReportLibraryItem) => void; onDelete: (item: ReportLibraryItem) => void; onOpen: (item: ReportLibraryItem) => void }) {
  ...
  return <article className="overflow-hidden rounded-[1.5rem] border border-ink/10 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-panel focus:ring-2 focus:ring-signal" id={`report-${item.reportId}`} tabIndex={-1}>
    <div className="relative aspect-video bg-ink/90">{item.playbackUrl ? <video className="h-full w-full object-cover opacity-85" muted playsInline preload="metadata" src={item.playbackUrl} /> : <div className="grid h-full place-items-center text-sm text-white/50">Preview unavailable</div>}<span className={`absolute left-3 top-3 rounded-full px-3 py-1.5 text-xs font-bold ${severityClass(item.severityLevel)}`}>Severity {item.severityLevel}</span><span className="absolute right-3 top-3 rounded-full bg-white/90 px-3 py-1.5 text-xs font-semibold text-ink">{statusLabel(item.status)}</span></div>
    <div className="p-5"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-moss">{item.incidentType}</p><h2 className="mt-2 line-clamp-2 text-xl font-semibold tracking-[-0.025em]">{item.title}</h2></div><span className="shrink-0 text-sm font-semibold text-ink/55">{Math.round(item.confidenceScore * 100)}%</span></div><p className="mt-3 line-clamp-3 text-sm leading-6 text-ink/60">{item.summary}</p><p className="mt-4 truncate text-xs text-ink/40">{item.filename} · {new Date(item.generatedAt).toLocaleString()}</p><p className="mt-1 truncate text-[11px] text-ink/35">{item.model}</p>
