...
| `.scripts/resolve-ssh-target.sh`  | — (new)                     | **laptop** | Sourced by `start.sh` / `tunnel.sh`: non-interactively resolves `VSS_SSH_TARGET` without prompting (`VSS_SSH_TARGET`, else `VSS_SSH_USER`, else `~/.ssh/config` `User` for `Host kwanz-ws`, else `$USER`/`whoami`; host from `VSS_SSH_HOST`, default `kwanz-ws`) |
