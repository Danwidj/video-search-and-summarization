...
if [ "${#docker_svcs[@]}" -gt 0 ]; then
  ...
  (cd "${compose_dir}" && docker compose -f compose.yml -f developer-profiles/dev-profile-incident/compose.override.yml --env-file "${generated_env}" -p mdx up -d --build --force-recreate "${docker_svcs[@]}")
fi
